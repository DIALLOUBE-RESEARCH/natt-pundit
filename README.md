...
**Tests (honest split):** CI / `npm test` covers MCP, contracts, keeper, fan orchestrator (suite PASS — see CI badge). **Escrow:** Rust unit suite for settle-critical `validate_stat` parse (`cargo test -p natt_escrow` / CI job `escrow-unit`) — live CPI fixture in-tree, no silent skip. **Not yet:** full localnet Anchor e2e (create/deposit/settle against TxLINE on validator). Settlement for jury = [live Solscan txs](./docs/JURY_VERIFICATION.md) + this parse suite + CPI smoke (`solana-escrow/scripts/smoke_view_validate.ts`).
