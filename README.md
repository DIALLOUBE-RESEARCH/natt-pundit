...
**Tests (honest split):** CI / `npm test` covers MCP, contracts, keeper, fan orchestrator. **Escrow:** Rust suite (`cargo test -p natt_escrow` / CI `escrow-unit`) — CPI `validate_stat` parse + settle/claim/refund/payout **money guards** (24+ unit tests). **Web:** Next.js **14.2.35** (patched 14.2 security line). **Not yet on CI:** full localnet BPF e2e vs live TxLINE program (needs Anchor CLI) — settlement end-to-end for jury = [live Solscan txs](./docs/JURY_VERIFICATION.md) + CPI smoke.
