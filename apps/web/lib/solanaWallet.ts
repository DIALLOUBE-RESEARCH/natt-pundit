...
async function prepareTx(tx: Transaction, payer: string, connection?: Connection): Promise<Transaction> {
...
export function buildLegacyInjectedWallet(): ConnectedWallet | null {
  const provider = getLegacyInjected();
  if (!provider?.signAndSendTransaction) return null;
  ...
  return {
    ...
    signAndSendTransaction: async (tx, connection) => {
      const ready = await prepareTx(tx, address, connection);
...
async function connectWalletStandard(wallet: Wallet): Promise<ConnectedWallet> {
  ...
  return {
    ...
    signAndSendTransaction: async (tx, connection) => {
      const ready = await prepareTx(tx, account.address, connection);
      ...
      if (signAndSendFeature) {
        const chain = connection ? "solana:devnet" : "solana:mainnet";
      ...
      const conn = connection ?? solanaConnection();
      return conn.sendRawTransaction(signedBytes, {
        skipPreflight: false,
