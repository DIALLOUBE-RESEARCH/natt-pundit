...
export function verifyTxlineSettlementProof(input: TxlineProofNodesInput): {
  ...
  const statLeaf = scoreStatLeafHash(stat.key, stat.value, stat.period);
  if (!verifyMerklePath(statLeaf, input.statProof, input.eventStatRoot)) {
  ...
  // Levels 1-2 cryptographically valid; main tree terminal matches declared root.
  if (buffersEqual(terminal.hash, input.merkleRoot)) {
